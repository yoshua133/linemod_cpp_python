...
public:
    ...
    static cv::Mat transform(cv::Mat src, float angle, float scale){
        ...
        cv::Mat rot_mat = cv::getRotationMatrix2D(center, angle, scale); // to get the rotation matrix 
        cv::warpAffine(src, dst, rot_mat, src.size());  // actually doing the rotation, only affect the dst, won't influence src
    ...
    void produce_infos(){
        ...
        if(angle_range.size() == 1 && scale_range.size() == 1){
            ...
            for(float scale = scale_range[0]; scale <= scale_range[1]+eps; scale += scale_step){
                for(float angle = angle_range[0]; angle <= angle_range[1]+eps; angle += angle_step){
                    infos.emplace_back(angle, scale); //类似于push back 在容器尾部增加一个元素
                }
            }
        }
    }
